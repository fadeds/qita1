'''
 变量名 DHHM   账号 多账号用&隔开
        ...
        LTDLC  代理池  多代理池用&隔开     有代理池的自己添加变量 没有的不用设置会使用本地IP
  
  启动时间设置在23:59分-0点之间 会等待到23:59:56秒开始抢任务 其他时间马上运行 
      如果提示任务无法完成就增加56秒这个时间 如果提示已领完就减小 
      建议挂代理池 同一IP 请求20次回请求频繁 
      一个本多号就挂代理池 
...
PHONE_V = os.getenv('DHHM', '17588888880&15288888888').split('&')
CSCS = int(os.getenv('CSCS', '2'))
LTDLC = os.getenv('LTDLC', '').split('&')


TASK_IDS = [125, 123, 122] 
infocode = {
    "121": "阅读15分钟任务",
    "122": "阅读120分钟任务",
    "123": "阅读240分钟任务",
...
TARGET_TIME = datetime.strptime("23:59:56.500", "%H:%M:%S.%f")
...
def process_task(token, task_id, proxies):
...
def get_aes_v_task(tokens, proxies):
    threads = []
    for task_id in TASK_IDS:
        for _ in range(REQ_NUM[task_id]):
            token = tokens[task_id]
            thread = threading.Thread(target=process_task, args=(token, task_id, proxies))
...
def main():
    ...
    if proxies and proxies != ['']:
        print(f"使用代理池: {proxies}")
    else:
        print("使用本地IP")

    for i, phone in enumerate(PHONE_V):
        proxy = proxies[i // 3 % len(proxies)] if proxies and proxies != [''] else None  
        thread = threading.Thread(target=login, args=(phone, proxy))
        threads.append(thread)
        thread.start()
